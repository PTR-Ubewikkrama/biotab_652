...
type TableSearchCommonProps = {
    searchFun: (from: Date | null, to: Date | null, filterType: string, filterValue: string, status: string) => void,
...
export default function TableSearchFormCommon({ searchFun, dropDownItems = { "ALL": "ALL", "TEST_ID": "TEST ID", "DEVICE_MAC": "DEVICE MAC" } }: TableSearchCommonProps) {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, actions) => {
            searchFun(from, to, filterVal, values.searchValue, status);
        }
    });
            ...
            <Grid container spacing={{ xs: 2, md: 3 }} columns={{ xs: 4, sm: 8, md: 12 }}>
                ...
                <Grid item xs={4} sm={4} md={2} >
                    <FormControl style={{ minWidth: "100%" }} sx={{ mt: 2 }}>
                        ...
                        >
                            {Object.entries({ "NONE": "NONE", "PASS": "PASS", "FAIL": "FAIL" }).map(([key, value]) => (
                                <MenuItem value={key} key={key}>{value}</MenuItem>
                            ))}
