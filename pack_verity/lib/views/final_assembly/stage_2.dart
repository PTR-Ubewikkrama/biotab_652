...
class StageTwoPage extends StatefulWidget {
  const StageTwoPage({super.key});

  @override
  State<StageTwoPage> createState() => _StageTwoPageState();
}

class _StageTwoPageState extends State<StageTwoPage> {
  ...
  void _submitForm() {
    ...
    Map<String, String> data = {
      ...
      'bladderCode': _controllers[1].text,
      'uplNumber': _controllers[2].text
    };

    faService.addFinalAssemblyStepTwo(data).then((value) {
                        ...
                        data: TestInputData(
                          ...
                          asyncValidatorFun: (val) async {
                            if (val!.isEmpty) {
                              ...
                              if (index == 1) {
                                bool isDeleting = previousValue != null &&
                                    previousValue!.length > val.length;

                                final firstLetter =
                                    faData!.category![0].toUpperCase();

                                if (!isDeleting) {
                                  if (val.length > 2 && !val.contains('-')) {
                                    String updatedVal =
                                        '${val.substring(0, 2)}-$firstLetter${val.substring(2)}';
                                    _controllers[index].text = updatedVal;
                                    _controllers[index].selection =
                                        TextSelection.fromPosition(
                                      TextPosition(offset: updatedVal.length),
                                    );
                                ...
                                if (!RegExp(
                                        r'^[A-Za-z0-9]{2}-$firstLetter.*$'
                                            .replaceFirst(
                                                r'$firstLetter', firstLetter),
                                        caseSensitive: false)
                                    .hasMatch(val)) {
                                  _errors[index] =
                                      'Invalid ${getStepTitles(index)} Code, Should be XX-$firstLetter...';
                                  return 'Invalid ${getStepTitles(index)} Code, Should be XX-$firstLetter...';
                                }
                              }
  ...
  Future<Map<String, dynamic>> validateByRemote(String val, int index) async {
    ...
    switch (index) {
      case 0:
        ValidateDeviceForStageTwoResponse? resp =
            await faService.validateDeviceForStageTwoById(data);
        if (resp != null && resp.isSuccess()) {
          setState(() {
            faData = resp.data!.finalAssembly;
          });
      ...
      case 1:
        CommonResponse? resp = await faService.validateBladderCode(data);
        if (resp != null && resp.isSuccess()) {
          return {'status': true, 'message': resp.statusDescription};
        } else {
      ...
      case 2:
        CommonResponse? resp = await faService.validateUPLNumber(data);
        if (resp != null && resp.isSuccess()) {
          return {'status': true, 'message': resp.statusDescription};
        } else {
          return {'status': false, 'message': resp!.statusDescription};
